...
def main(week_day, pklot_ID, week_shift):
    print("Starting TEST for day: ",week_day," in parking: ", pklot_ID)
    df = ct.csv_open("DATASET/PARKING_LOTS/occupancy_park_"+str(pklot_ID)+".csv",sep=",").getDataFrame()
    weeks_train_number = 18 - week_shift
    look_back = 6
    model,trend = generate_model(df,weeks_train_number,pklot_ID,week_day=week_day,look_back=look_back, force_train=False)
    p_input = trend[:len(trend)-week_shift]
    g_t = trend[-1]
    g_t = g_t[look_back:len(g_t)-look_back]
    prediction, _ = predict(model,p_input,weeks_mean_number=4,look_back=look_back,weeks_shift=week_shift)
    pr = []
    for pred in prediction:
        pr.append(pred[0])
    out = {}
    mse = mean_squared_error(pr,g_t)
    print("MSE:",mse)
    t = 0
    for pred in pr:
        out[secToTime(t,clockFormat=True)] = str(pred)
    ...
    plt.plot(g_t,label="ground_truth")
    plt.legend()
    # plt.show(block=True)
    plt.savefig("PLOTS/"+str(week_day)+"_"+str(pklot_ID)+".svg",format="svg")
    plt.close() 
    return mse
...
if __name__ == "__main__":
    ...
    mse_2 = []
    for j in range(ws):
        mse_2.append(main(3,11,j))

    print(mse_2)

    plt.bar(list(range(1,ws+1)),mse_2,label="mse")
    plt.legend()
